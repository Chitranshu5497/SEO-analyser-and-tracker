...
export default function RankDetail() {
    ...
    const drawChart = () => {
        ...
        // Draw glow layer behind the line (extra blurred stroke pass)
        ctx.save();
    ...
    if (!tracking) {
        return (
            <div className="min-h-screen bg-background flex items-center justify-center">
                <div className="relative text-center glass-strong border border-danger/20 rounded-2xl p-10 shadow-[0_0_35px_-10px_rgba(239,68,68,0.35)] overflow-hidden">
                    ...
                    <h2 className="relative text-xl font-bold text-foreground mb-2">Tracking Not Found</h2>
                    <Link
                        to="/rank-tracker"
                        className="relative bg-primary px-5 py-2.5 rounded-xl text-sm font-semibold text-primary-foreground mt-4 inline-block shadow-[0_0_18px_rgba(99,102,241,0.5)] hover:shadow-[0_0_28px_rgba(99,102,241,0.7)] transition-all"
        ...
        <div className="relative min-h-screen pt-16 md:pt-24 bg-background overflow-hidden">
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="mb-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <button
                            ...
                            className="glass border border-border px-4 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 hover:border-primary/30 hover:shadow-[0_0_20px_-6px_rgba(99,102,241,0.5)] transition-all disabled:opacity-50 self-start text-foreground"
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6" style={{ animationDelay: "100ms" }}>
                    <div className="relative glass-strong border border-primary/20 rounded-2xl p-6 text-center shadow-[0_0_25px_-10px_rgba(99,102,241,0.4)] overflow-hidden">
                        <div className="absolute -top-8 -right-8 w-24 h-24 bg-primary/15 blur-2xl rounded-full pointer-events-none"></div>
                        <p className="relative text-xs text-muted-foreground mb-2 flex items-center justify-center gap-1">
                            <Target size={14} />
                            Current Position
                        </p>
                        {tracking.status === "checking" ? (
                            <Loader2 size={32} className="relative animate-spin mx-auto text-primary" />
                        ) : (
                            <p className={`relative text-4xl font-bold ${getPositionColor(tracking.currentPosition)}`}>{tracking.currentPosition ? `#${tracking.currentPosition}` : "Not Ranked"}</p>
                        )}
                        {tracking.currentPage && <p className="relative text-xs text-muted-foreground mt-1">Page {tracking.currentPage}</p>}
                    </div>

                    <div className="glass border border-border rounded-2xl p-6 text-center hover:border-primary/25 hover:shadow-[0_0_20px_-8px_rgba(99,102,241,0.35)] transition-all">
                    ...
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${
                                activeTab === tab.id ? "bg-primary text-primary-foreground shadow-[0_0_18px_-4px_rgba(99,102,241,0.6)]" : "glass border border-border text-muted-foreground hover:text-foreground hover:border-primary/25"
                            }`}
                ...
                <div key={activeTab}>
                    {activeTab === "overview" && (
                        <div className="space-y-6">
                            {/* Ranking Chart */}
                            <div className="relative glass border border-border rounded-2xl p-6 overflow-hidden">
                                <div className="absolute -top-10 -right-10 w-40 h-40 bg-primary/10 blur-[60px] rounded-full pointer-events-none"></div>
                                <h3 className="relative text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                                ...
                                {tracking.rankHistory.filter((h) => h.position !== null).length > 0 ? (
                                    ...
                                    <div className="relative text-center py-12 text-muted-foreground">
                                        <Calendar size={32} className="mx-auto mb-2 opacity-50" />
                                        <p className="text-sm">No ranking data yet. Check back after the daily tracking runs.</p>
                                    </div>
                                )}
                                <p className="relative text-xs text-muted-foreground mt-3 text-center">↑ Lower position number = higher rank. Updated daily at 9:00 AM UTC.</p>
                            </div>
                                ...
                                <div className="glass border border-border rounded-2xl p-6">
                                    ...
                                    <div className="space-y-2">
                                        {tracking.competitors.slice(0, 3).map((comp, i) => (
                                            <div key={i} className="glass border border-border rounded-xl p-4 flex items-start gap-4 hover:border-primary/25 hover:shadow-[0_0_18px_-8px_rgba(99,102,241,0.4)] transition-all">
                                                <div
                                                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
                                                        i === 0
                                                            ? "bg-amber-500/15 text-amber-400 border border-amber-500/30 shadow-[0_0_12px_-3px_rgba(245,158,11,0.5)]"
                                                            : i === 1
                            ...
                            {tracking.competitors.length > 0 ? (
                                <div className="space-y-3">
                                    {tracking.competitors.map((comp, i) => (
                                        <div key={i} className="glass border border-border rounded-xl p-4 hover:border-primary/25 hover:shadow-[0_0_20px_-8px_rgba(99,102,241,0.4)] transition-all">
                                            <div className="flex items-start gap-4">
                                                <div
                                                    className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 ${
                                                        comp.position <= 3
                                                            ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 shadow-[0_0_12px_-3px_rgba(16,185,129,0.5)]"
                                                            : comp.position <= 10
                                                            ? "bg-primary/15 text-primary border border-primary/30 shadow-[0_0_12px_-3px_rgba(99,102,241,0.5)]"
                                                ...
                                                <a href={comp.url} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg hover:bg-primary/10 text-muted-foreground hover:text-primary transition-all shrink-0">
                            ...
                            {tracking.rankHistory.length > 0 ? (
                                <div className="space-y-2">
                                    {[...tracking.rankHistory]
                                        ...
                                        .map((entry, i) => (
                                            <div key={i} className="glass border border-border rounded-xl p-4 flex items-center gap-4 hover:border-primary/25 hover:shadow-[0_0_18px_-8px_rgba(99,102,241,0.4)] transition-all">
                                                <div
